 ...
 * Sample page
 */
public class KosguSpravochnik extends Page {
	
	private String table_loc = ".//div[contains(., 'Основное табличное представление')]";//".//th[contains(.,'Код')]/../../../../..";
  ...
  public KosguSpravochnik(WebDriver webDriver) {
    super(webDriver);
  }
...
//Left
  ...
  public void clearFilter(){
	  ...
	  if(filter.getAttribute("src").contains("filter_off")){
		  ...
		  if(flag) {row.sendKeys(Keys.ENTER);}
...
//	  max = getMaxPage();
	  for(int i = cur;i <= max;i++){
		  wait.until(ExpectedConditions.elementToBeClickable(By.xpath(rows_loc)));//elementToBeClickable
		  ...
		  for (Iterator<WebElement> iterator = list.iterator(); iterator.hasNext();) {
			...
			if(row_val.equals(val)){
				...
				row.findElement(By.xpath(".//td[2]/div")).click();
				my_generic_wait_for_page_load();
				clickOk();
				return;
			}
